 ...
 *
 */
public class CurrentCommunityIDDataTag extends APTURLDODataTag
{
	...
	static
	{
		...
		TAG = new TagMetaData("currentcommunityiddata",
			COMMON_PRETAGMETADATA_DESC +
			"Gets the current community id " +
	...
	public HTMLElement DisplayTag()
	{
		...
		if( TaskAPIUICommon.GetEnvTypeObject(env) != null ) 
		{
			int nCurrCommID = TaskAPIUICommunity.GetCurrentCommunityID(currSpace);
			int nCurrPageID = TaskAPIUICommunity.GetCurrentCommunityPageID(currSpace);
			String sCurrCommName = TaskAPIUICommunity.GetCurrentCommunityName(currSpace);
			IPTURL url = TaskAPIUICommunity.CreateCommunityPageCommonOpenerURL(currSpace, nCurrCommID, nCurrPageID, sCurrCommName);	
		...
		{
			return new HTMLComment("Unable to determine current community");
		}
	...
	public ATag Create()
	{
		return new CurrentCommunityIDDataTag();
	}
